...
contract Wallet_Test_01 is BaseTest {
    ...
    function test_proofOfConcept() public {
        ...
        params[1] = abi.encode(
            currency0,
            currency1,
            address(proxyWallet) //退还本金和奖励地址(即代理钱包地址)
        );
    ...
    function test_transferFromWallet() public {
        ...
        assertApproxEqAbs(
            curForTransfer.balanceOf(address(beneficiary)), WANT_TO_TRANSFER, WANT_TO_TRANSFER * SLIPPAGE_BPS / 10_000
        );
    }
}